...
int main(){
    ...
    LQR robot_motion_LQR(N);//求解Riccati矩阵 P 时 迭代N次
    ...
    for(int i = 0;i<350;i++){
     ...
     // 传入机器人状态、参考轨迹、min_index, A , B , Q, R     求解得到前轮转角的增量
        double delta = robot_motion_LQR.LQRControl(robot_state, referencePath.ref_path, s0, state_space[0], state_space[1], Q, R);// 前轮转角
        delta += ref_delta;
